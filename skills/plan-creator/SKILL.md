...
8. **Hand off only when execution is deterministic.** Pass to `execute-plan` only when the active slice is singular, bounded, named by files/surfaces, validation is explicit, stop condition is explicit, and machine truth is parseable when required.
9. **Stay in planning when ambiguity remains.** If multiple next slices compete, validation changes, owner boundary changes, or parser truth drifts, keep refining instead of pretending the pack is ready.
